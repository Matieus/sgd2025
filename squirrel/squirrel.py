...
MOVERATE = 9  # how fast the player moves (base speed, can increase with levels)
...
SQUIRRELMINSPEED = 3  # slowest squirrel speed (base speed, can increase with levels)
SQUIRRELMAXSPEED = 7  # fastest squirrel speed (base speed, can increase with levels)
DIRCHANGEFREQ = 2  # % chance of direction change per frame
LEFT = "left"
RIGHT = "right"

LEVELUPTIME = 2  # how long the "Level Up" text stays on the screen in seconds
...
def runGame():
    ...
    # reset dynamic variables for each new game
    SQUIRRELMINSPEED = 3
    SQUIRRELMAXSPEED = 7
    ...
    levelUpSurf = BASICFONT.render("Poziom w górę!", True, WHITE)
    levelUpRect = levelUpSurf.get_rect()
    levelUpRect.center = (HALF_WINWIDTH, HALF_WINHEIGHT - 40)
    ...
    while True:  # main game loop
        ...
        # draw current level
        levelSurf = BASICFONT.render(f"Poziom: {level}", True, WHITE)
        levelRect = levelSurf.get_rect()
        levelRect.midtop = (HALF_WINWIDTH, 5)
        DISPLAYSURF.blit(levelSurf, levelRect)

        # draw "Level Up" text if applicable
        if levelUpMode:
            DISPLAYSURF.blit(levelUpSurf, levelUpRect)
            if time.time() - levelUpStartTime > LEVELUPTIME:
                levelUpMode = False
        ...
        if not gameOverMode:
            ...
            if moveLeft:
                playerObj["x"] -= playerMoveRate
            if moveRight:
                playerObj["x"] += playerMoveRate
            if moveUp:
                playerObj["y"] -= playerMoveRate
            if moveDown:
                playerObj["y"] += playerMoveRate
            ...
            for i in range(len(squirrelObjs) - 1, -1, -1):
                ...
                if "rect" in sqObj and playerObj["rect"].colliderect(sqObj["rect"]):
                    ...
                    if sqObj["width"] * sqObj["height"] <= playerObj["size"] ** 2:
                        ...
                        # update score and handle level up
                        score += 1
                        if score % 5 == 0:
                            level += 1
                            levelUpMode = True
